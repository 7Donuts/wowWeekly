...
const VERSIONS = [
  {
    ...
    entries: [
      { type: 'new', text: '⚔️ BiS Gear modal — browse and import Best in Slot gear by class and specialization', detail: 'A new BiS Gear button in the toolbar opens a three-step modal: pick your class (13 classes with color-coded borders and official class icons), pick your spec (role icons for DPS/Heal/Tank, "New" badge for Devourer Demon Hunter), then browse a full slot-by-slot gear list. Check any items and click Import — each becomes a trackable task in Your List under a dedicated Best in Slot Gear section. Already-imported items are pre-checked on re-open so you never get duplicates.' },
      { type: 'new', text: 'Complete BiS data for all 40 specs — 627 items across every class for Patch 12.0.5', detail: 'Full BiS lists are populated for all 40 playable specs including Death Knight (Blood, Frost, Unholy), Demon Hunter (Havoc, Vengeance, Devourer), Druid (Balance, Feral, Guardian, Restoration), Evoker (Augmentation, Devastation, Preservation), Hunter (Beast Mastery, Marksmanship, Survival), Mage (Arcane, Fire, Frost), Monk (Brewmaster, Mistweaver, Windwalker), Paladin (Holy, Protection, Retribution), Priest (Discipline, Holy, Shadow), Rogue (Assassination, Outlaw, Subtlety), Shaman (Elemental, Enhancement, Restoration), Warlock (Affliction, Demonology, Destruction), and Warrior (Arms, Fury, Protection). Each item includes the exact boss or source and the raid/dungeon it comes from.' },
      { type: 'new', text: 'Gear slot icons — WoW paperdoll slot art shown in both the BiS modal and imported task cards', detail: 'Every BiS gear row in the modal and every imported task card displays the official WoW gear slot icon (head, neck, shoulders, back, chest, wrists, hands, waist, legs, feet, finger, trinket, main hand, off hand). Hovering the icon shows the slot name. The back slot uses the chest slot icon as intended.' },
      { type: 'new', text: 'Role icons — DPS, Healer, and Tank icons displayed next to each spec in the spec picker', detail: 'The spec selection screen now shows the official WoW role icons next to each specialization name so you can instantly identify the role. Specs without BiS data are dimmed to indicate they are coming soon.' },
      { type: 'new', text: 'BiS as a separate system — gear tasks have their own section and do not affect weekly progress', detail: 'Imported BiS items appear under a dedicated Best in Slot Gear header in Your List, completely separate from regular tasks and the Custom Tasks section. BiS tasks are excluded from the weekly progress bar (X / Y done), the weekly history/stats snapshot, the Summary modal, and the Custom Tasks section in the All view. They remain individually completable so you can check off items as you acquire them.' },
      { type: 'new', text: '✏️ Edit button on BiS task cards — replace any item with your own personal BiS choice', detail: 'Every BiS task card in Your List has a pencil edit button. Clicking it opens a pre-filled edit modal showing the locked slot prefix (e.g. [Head]) and editable item name and source fields. Save updates the card immediately while preserving the slot icon. Use this to swap in a different item if your actual BiS situation differs from the default list — for example, a different trinket or a crafted alternative.' },
    ]
  },
  {
    version: 'v1.8.4',
