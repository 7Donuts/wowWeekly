...
function toggle(id, taskEl) {
  ...
  // And tell the server, which is what makes this survive being done on two
  // devices. Optimistic: the local write above already happened, so the
  // checkbox does not wait for a round trip.
  if (typeof observeTask === 'function') {
    observeTask(currentChar, id, { done: !wasDown }, 'member');
  }
  if (!wasDown) {
...
function adjustGoal(id, max, delta) {
  ...
  saveDone(done);
  // A counter the member sets by hand is a correction, so the server takes it
  // as given rather than merging it upward. `done` only travels for a capped
  // goal: an uncapped one never auto-checks, here or there.
  if (typeof observeTask === 'function') {
    const fields = { value: next };
    if (!uncapped) fields.done = next >= max;
    observeTask(currentChar, id, fields, 'member');
  }
  render();
}
